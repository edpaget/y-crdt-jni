...
The rdm MCP server is connected and provides tools for plan repo operations. Most tools require a `project` parameter — use "y-crdt-jni" for the current project.

## Discovering work

- `rdm_roadmap_list` with `project: "y-crdt-jni"` — list all roadmaps with progress
- `rdm_task_list` with `project: "y-crdt-jni"` — list open/in-progress tasks
- `rdm_task_list` with `project: "y-crdt-jni", status: "all"` — list all tasks including done

## Reading details

- `rdm_roadmap_show` with `project: "y-crdt-jni", roadmap: "<slug>"` — show roadmap with phases and body
- `rdm_phase_list` with `project: "y-crdt-jni", roadmap: "<slug>"` — list phases with numbers and statuses
- `rdm_phase_show` with `project: "y-crdt-jni", roadmap: "<slug>", phase: "<stem-or-number>"` — show phase details
- `rdm_task_show` with `project: "y-crdt-jni", task: "<slug>"` — show task details

## Searching

Use `rdm_search` for fuzzy matching against titles and body content:

- `rdm_search` with `query: "auth", project: "y-crdt-jni"` — find items mentioning "auth"
- `rdm_search` with `query: "index", kind: "task", project: "y-crdt-jni"` — find only tasks
- `rdm_search` with `query: "auth", status: "in-progress", project: "y-crdt-jni"` — filter by status

## Updating status

- `rdm_phase_update` with `project: "y-crdt-jni", roadmap: "<slug>", phase: "<stem-or-number>", status: "done"`
- `rdm_task_update` with `project: "y-crdt-jni", task: "<slug>", status: "done"`

## Creating items

- `rdm_roadmap_create` with `project: "y-crdt-jni", slug: "<slug>", title: "Title", body: "Summary."`
- `rdm_phase_create` with `project: "y-crdt-jni", roadmap: "<slug>", slug: "<slug>", title: "Title", number: <n>, body: "Details."`
- `rdm_task_create` with `project: "y-crdt-jni", slug: "<slug>", title: "Title", body: "Description."`
...
Use `rdm_roadmap_list` with `project: "y-crdt-jni"` to see all roadmaps and their progress. Check `rdm_task_list` with `project: "y-crdt-jni"` for open tasks. Identify what is in-progress and what comes next before writing any code.

### Implementing a roadmap phase

1. Read the phase: `rdm_phase_show` with `project: "y-crdt-jni", roadmap: "<slug>", phase: "<stem-or-number>"`
...
4. Include a `Done:` line in the git commit message — the post-merge hook will mark the phase done and record the commit SHA.
   **Use the exact roadmap slug and phase stem from the rdm tools above — do NOT invent or paraphrase them:**
   ...
   ```
5. Check the next phase: `rdm_phase_list` with `project: "y-crdt-jni", roadmap: "<slug>"`
...
`rdm_task_create` with `project: "y-crdt-jni", slug: "<slug>", title: "Description of the issue", body: "Details."`
...
`rdm_task_promote` with `project: "y-crdt-jni", task: "<task-slug>", roadmap_slug: "<new-roadmap-slug>"`
